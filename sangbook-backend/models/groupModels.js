...
export const addMember = async (group_id, user_id) => {
    await db.execute("INSERT INTO group_members (group_id, user_id) values(?,?)",
        [group_id, user_id]
    );
}
...
export const sendMessage = async (group_id, sender_id, message, media) => {
    ...
    await db.execute(
        "INSERT INTO group_messages (group_id, sender_id, message, media) VALUES (?, ?, ?, ?)",
...
export const getGroupMessages = async (group_id) => {
    const [rows] = await db.execute("SELECT gm.*, u.username, u.avatar from group_messages gm JOIN users u ON gm.sender_id = u.id where gm.group_id = ? order by gm.sent_at ASC",
        [group_id]
    );
    return rows;
}
